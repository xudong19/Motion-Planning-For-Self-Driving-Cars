...
class LocalPlanner:
    def __init__(self, num_paths, path_offset, circle_offsets, circle_radii, 
        ...
        self._prev_best_path = np.asarray([np.zeros(100), np.zeros(100), np.zeros(100)])
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        #     heading = np.arctan2(delta_y / delta_x)
        # except Exception:
        #     heading = (np.pi / 2) * np.sign(delta_x) * np.sign(delta_y)
        ...
        goal_x = (rot_matrix @ np.array(goal_state_local[:2]))[0]
        goal_y = (rot_matrix @ np.array(goal_state_local[:2]))[1]
        # ------------------------------------------------------------------
