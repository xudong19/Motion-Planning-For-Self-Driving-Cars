    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                y_loc = path[1][j]
                yaw_loc = path[2][j]
                circle_locations[:, 0] = x_loc + np.array(self._circle_offsets)*np.cos(yaw_loc)
                circle_locations[:, 1] = y_loc + np.array(self._circle_offsets)*np.sin(yaw_loc)
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                xp, yp = paths[i][0][-1], paths[i][1][-1]
                xg, yg, *_ = goal_state
                score = np.sqrt((xp - xg) ** 2 + (yp - yg) ** 2)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            xp2, yp2 = paths[j][0][-1], paths[j][1][-1]
                            score += self._weight * 1 / np.sqrt((xp - xp2) ** 2 + (yp - yp2) ** 2)
                            # --------------------------------------------------
